export type Lang = "en" | "es";

export const translations = {
  en: {
    home: "Home",
    about: "About Me",
    projects: "Projects",
    skills: "Skills",
    contact: "Contact",
    title: "Portfolio Walen I Calderon",
    subtitle: "Computer Science student seeking internships",
    aboutTag: "Building real systems while studying, working, and improving my technical foundation.",
    aboutJourneyTitle: "My path",
    aboutJourneyText:
      "I'm a Computer Science student in New Jersey building a technical foundation through Java, full-stack web projects, Linux, and infrastructure practice. My main project is Gestion Truck, a fleet and logistics MVP that connects backend APIs, dashboard UI, authentication, deployment, and product planning.",
    aboutGoalTitle: "What I'm aiming for",
    aboutGoalText:
      "I'm looking for internship opportunities where I can grow as a practical software engineer, contribute with discipline, and keep improving in backend systems, full-stack development, databases, Linux, Docker, and infrastructure.",
    projectsTag: "A few things I'm building and refining as part of my learning process.",
    projectReadMore: "Read more",
    projectShowLess: "Show less",
    projectComingSoon: "Coming soon — links will be added when the project is public.",
    projectOpenNewTab: "Opens in a new tab",
    projectClickToExpand: "Click to expand",
    projectClickToCollapse: "Click to collapse",
    projectExpandHint: "Expand",
    projectLabelProblem: "Problem",
    projectLabelFull: "Overview",
    projectLabelRole: "My role",
    projectLabelTech: "Technologies",
    projectLabelFeatures: "Highlights",
    projectLiveDemo: "Live demo",
    projectGitHub: "GitHub",
    projectViewDetails: "View details",
    skillsTag: "A practical stack I am using while building full-stack SaaS and logistics workflows.",
    skillTechTitle: "Technical skills",
    skillSoftTitle: "Strengths",
    skillStackTitle: "Core stack",
    skillBackendTitle: "Backend",
    skillFrontendTitle: "Frontend",
    skillDevOpsTitle: "DevOps & infrastructure",
    skillInfrastructureTitle: "Infrastructure",
    skillProductTitle: "Product domain",
    skillAuthTitle: "Authentication & security",
    skillLogisticsTitle: "Logistics & fleet management",
    skillEngineeringTitle: "Collaboration & engineering",
    skillPlatformTitle: "Gestion Truck roadmap",
    skillPlatformText:
      "Gestion Truck is being built as a fleet and logistics platform. Current focus: administration, driver workflows, shipment tracking, and a clear path toward future ELD/e-logbook integrations.",
    skillOne: "Problem solving",
    skillTwo: "Curiosity across tech",
    skillFive: "Java foundation",
    skillSix: "Software and hardware interest",
    skillThree: "Continuous learning",
    skillFour: "Git workflow",
    contactTag: "You can reach me through email, GitHub, or LinkedIn.",
    contactText:
      "I'm open to internship opportunities, feedback, and conversations about software, programming, or hardware-related work while I keep growing as a developer.",
    all: "All",
    academic: "Academic",
    proj1: "Console app for teams and stats (parsing, FileWriter, Scanner).",
    proj1_preview: "Java CLI to manage teams, rosters, and game statistics from the terminal.",
    proj1_problem:
      "Practice real-world file I/O, parsing, and structured data without building a GUI yet.",
    proj1_role: "Solo developer: requirements, implementation, and manual testing.",
    proj1_features:
      "Team roster handling|Read/write persistence with FileWriter|Scanner-driven console UX",
    proj4: "Console app to create, edit and list tasks (OOP, files).",
    proj4_preview: "Terminal task manager built with Java OOP and file-backed storage.",
    proj4_problem:
      "Model tasks cleanly with objects and persist them reliably between sessions.",
    proj4_role: "Solo developer for design, coding, and validation.",
    proj4_features:
      "Create, edit, and list tasks|Encapsulated domain model|Save/load state to files",
    proj7: "Business website with Firebase integration for data management.",
    proj7_preview: "Company website wired to Firebase for manageable content and data.",
    proj7_problem:
      "Deliver a fast, professional web presence with a lightweight backend for updates.",
    proj7_role: "Full-stack: layout, components, Firebase integration, and deploy.",
    proj7_features:
      "Responsive marketing pages|Firebase for data|Optimized for production hosting",
    proj8: "A fleet and logistics management platform MVP focused on administration, alerts, driver operations, shipment tracking, and future ELD/e-logbook integrations.",
    proj8_preview: "Read-only MVP demo for fleet administration, alerts, driver operations, and logistics workflows.",
    proj8_problem:
      "Design a real operations platform around authentication, role-based access, fleet data, shipment workflows, and scalable dashboard UX.",
    proj8_role: "Full-stack developer across API architecture, database design, authentication flows, and operator-facing UI.",
    proj8_features:
      "Fleet and driver management|Shipment and route tracking|RBAC and protected workflows|Planned ELD/e-logbook integrations",
    experience: "Experience",
    expHelp: "Relevant projects and roles.",
    exp1: "Academic Java projects (CLI), small web apps and Git practice.",
    exp2: "Accessible calculator, localStorage todo and bilingual portfolio.",
    education: "Education",
    eduHelp: "Academic background and current learning focus.",
    edu1: "Computer Science coursework with a focus on programming fundamentals, problem solving, math, and software development foundations.",
    eduFocusTitle: "Current technical focus",
    eduFocusText:
      "Building Gestion Truck, improving backend/API skills, learning deployment through Raspberry Pi infrastructure, and keeping project documentation organized.",
    downloadCV: "Download CV",
    heroBadge: "Available for new projects",
    heroRole:
      "Computer Science student in New Jersey building full-stack Java and web projects, with a focus on SaaS-style logistics systems, backend APIs, Linux, and deployable infrastructure.",
    heroCtaViewProjects: "View projects",
    heroCtaDownloadCv: "Download CV",
    heroStatsGithubReposLabel: "Public GitHub repos",
    heroStatsCodingYearsLabel: "Coding experience",
    heroStatsCodingYearsValue: "2+",
    heroStatsStackLabel: "Stack focus",
    heroStatsStackValue: "Java • Spring • React",
    heroSpecialtiesLabel: "Specialties",
    contactMessage:
      "If you have an opportunity, an idea, or just want to say hi, my inbox is always open. I'll try my best to get back to you as soon as possible.",
    formName: "Name",
    formEmail: "Email",
    formMessage: "Message",
    formNamePlaceholder: "Your name",
    formEmailPlaceholder: "you@email.com",
    formMessagePlaceholder: "How can I help you?",
    formSubmit: "Send message",
    formSubmitting: "Sending...",
    formSuccess: "Sent!",
    formError: "Error sending",
    contactSuccessMsg: "Thanks for reaching out! I'll get back to you soon.",
    contactErrorMsg:
      "There was a problem. Please check the console or try emailing me directly.",
  },

  es: {
    home: "Inicio",
    about: "Sobre mí",
    projects: "Proyectos",
    skills: "Habilidades",
    contact: "Contacto",
    title: "Portfolio Walen I Calderon",
    subtitle: "Estudiante de Ciencias de la Computación en busca de internships",
    aboutTag: "Construyendo sistemas reales mientras estudio, trabajo y fortalezco mi base técnica.",
    aboutJourneyTitle: "Mi camino",
    aboutJourneyText:
      "Soy estudiante de Ciencias de la Computación en New Jersey y estoy construyendo una base técnica con Java, proyectos web full-stack, Linux e infraestructura. Mi proyecto principal es Gestion Truck, un MVP de flotas y logística que conecta APIs backend, dashboard, autenticación, despliegue y planificación de producto.",
    aboutGoalTitle: "Hacia dónde voy",
    aboutGoalText:
      "Estoy buscando oportunidades de internship donde pueda crecer como desarrollador práctico, aportar con disciplina y seguir mejorando en backend, full-stack, bases de datos, Linux, Docker e infraestructura.",
    projectsTag: "Algunas cosas que estoy construyendo y mejorando como parte de mi proceso de aprendizaje.",
    projectReadMore: "Leer más",
    projectShowLess: "Ver menos",
    projectComingSoon:
      "Próximamente — enlaces cuando el proyecto sea público.",
    projectOpenNewTab: "Se abre en una pestaña nueva",
    projectClickToExpand: "Clic para expandir",
    projectClickToCollapse: "Clic para contraer",
    projectExpandHint: "Expandir",
    projectLabelProblem: "Problema",
    projectLabelFull: "Resumen",
    projectLabelRole: "Mi rol",
    projectLabelTech: "Tecnologías",
    projectLabelFeatures: "Destacados",
    projectLiveDemo: "Demo en vivo",
    projectGitHub: "GitHub",
    projectViewDetails: "Ver detalles",
    skillsTag: "Stack práctico que estoy usando para construir SaaS full-stack y flujos logísticos.",
    skillTechTitle: "Habilidades técnicas",
    skillSoftTitle: "Fortalezas",
    skillStackTitle: "Stack principal",
    skillBackendTitle: "Backend",
    skillFrontendTitle: "Frontend",
    skillDevOpsTitle: "DevOps e infraestructura",
    skillInfrastructureTitle: "Infraestructura",
    skillProductTitle: "Dominio del producto",
    skillAuthTitle: "Autenticación y seguridad",
    skillLogisticsTitle: "Logística y gestión de flotas",
    skillEngineeringTitle: "Colaboración e ingeniería",
    skillPlatformTitle: "Roadmap de Gestion Truck",
    skillPlatformText:
      "Gestion Truck se está construyendo como una plataforma de flotas y logística. Enfoque actual: administración, flujos de conductores, seguimiento de envíos y una ruta clara hacia futuras integraciones ELD/e-logbook.",
    skillOne: "Resolución de problemas",
    skillTwo: "Curiosidad por la tecnología",
    skillFive: "Base en Java",
    skillSix: "Interés en software y hardware",
    skillThree: "Aprendizaje continuo",
    skillFour: "Flujo con Git",
    contactTag: "Puedes contactarme por email, GitHub o LinkedIn.",
    contactText:
      "Estoy abierto a oportunidades de internship, feedback y conversaciones sobre software, programación o trabajos relacionados con hardware mientras sigo creciendo como desarrollador.",
    all: "Todos",
    academic: "Académicos",
    proj1: "App de consola para equipos y estadísticas (parsing, FileWriter, Scanner).",
    proj1_preview: "CLI en Java para gestionar equipos, plantillas y estadísticas desde consola.",
    proj1_problem:
      "Practicar E/S de archivos, parsing y datos estructurados sin interfaz gráfica.",
    proj1_role: "Desarrollador único: requisitos, implementación y pruebas manuales.",
    proj1_features:
      "Gestión de plantillas|Persistencia con FileWriter|UX por consola con Scanner",
    proj4: "Aplicación de consola para crear, editar y listar tareas (POO, archivos).",
    proj4_preview: "Gestor de tareas en terminal con POO y almacenamiento en archivos.",
    proj4_problem:
      "Modelar tareas con objetos y guardar el estado entre sesiones de forma fiable.",
    proj4_role: "Desarrollador único en diseño, código y validación.",
    proj4_features:
      "Crear, editar y listar tareas|Modelo encapsulado|Guardar/cargar estado en archivos",
    proj7: "Sitio web empresarial con integración de Firebase para la gestión de datos.",
    proj7_preview: "Web corporativa con Firebase para contenidos y datos gestionables.",
    proj7_problem:
      "Ofrecer presencia web profesional con un backend ligero para actualizaciones.",
    proj7_role: "Full-stack: maquetación, Firebase y despliegue.",
    proj7_features:
      "Páginas responsive|Integración Firebase|Listo para hosting productivo",
    proj8: "MVP de plataforma de gestión de flotas y logística enfocado en administración, alertas, operaciones de conductores, seguimiento de envíos y futuras integraciones ELD/e-logbook.",
    proj8_preview: "Demo MVP de solo lectura para administración de flotas, alertas, operaciones de conductores y flujos logísticos.",
    proj8_problem:
      "Diseñar una plataforma de operaciones real con autenticación, control por roles, datos de flota, flujos de envío y UX escalable tipo dashboard.",
    proj8_role: "Desarrollador full-stack en arquitectura de API, diseño de base de datos, autenticación e interfaz para operadores.",
    proj8_features:
      "Gestión de flotas y conductores|Seguimiento de envíos y rutas|RBAC y flujos protegidos|Integraciones ELD/e-logbook planificadas",
    experience: "Experiencia",
    expHelp: "Proyectos y roles relevantes.",
    exp1: "Proyectos académicos en Java (CLI), apps web pequeñas y prácticas de Git.",
    exp2: "Calculadora accesible, lista de tareas con localStorage y portfolio bilingüe.",
    education: "Educación",
    eduHelp: "Formación académica y enfoque actual de aprendizaje.",
    edu1: "Formación en Computer Science con enfoque en fundamentos de programación, resolución de problemas, matemáticas y bases de desarrollo de software.",
    eduFocusTitle: "Enfoque técnico actual",
    eduFocusText:
      "Construir Gestion Truck, mejorar en backend/APIs, aprender despliegue con infraestructura en Raspberry Pi y mantener documentación técnica organizada.",
    downloadCV: "Descargar CV",
    heroBadge: "Disponible para nuevos proyectos",
    heroRole:
      "Estudiante de Ciencias de la Computación en New Jersey construyendo proyectos full-stack en Java y web, con enfoque en sistemas SaaS de logística, APIs backend, Linux e infraestructura desplegable.",
    heroCtaViewProjects: "Ver proyectos",
    heroCtaDownloadCv: "Descargar CV",
    heroStatsGithubReposLabel: "Repos públicos (GitHub)",
    heroStatsCodingYearsLabel: "Experiencia programando",
    heroStatsCodingYearsValue: "2+",
    heroStatsStackLabel: "Enfoque de stack",
    heroStatsStackValue: "Java • Spring • React",
    heroSpecialtiesLabel: "Especialidades",
    contactMessage:
      "Si tienes una oportunidad, una idea, o simplemente quieres saludar, mi bandeja de entrada siempre está abierta. Me esforzaré por responderte lo antes posible.",
    formName: "Nombre",
    formEmail: "Correo",
    formMessage: "Mensaje",
    formNamePlaceholder: "Tu nombre",
    formEmailPlaceholder: "tu@correo.com",
    formMessagePlaceholder: "¿En qué puedo ayudarte?",
    formSubmit: "Enviar mensaje",
    formSubmitting: "Enviando...",
    formSuccess: "¡Enviado!",
    formError: "Error al enviar",
    contactSuccessMsg: "¡Gracias por escribirme! Me pondré en contacto contigo pronto.",
    contactErrorMsg:
      "Hubo un problema. Por favor revisa la consola o intenta directamente a mi correo.",
  },
} as const;
